...
module.exports = function zalgoify(text, opts){
        opts = opts || {};

        length = opts.length || 2000;

        var up = opts.up || true;
        var mid =  opts.mid || true;
        var down = opts.down || true;

        var mini = false;
        var normal = opts.normal || true;
        var DANGERZONE = opts.DANGERZONE || false;
        ...
        for(var i=0; i<txt.length; i++){
                ...
                if(mini){
                        ...
                        var max = parseInt(((length - text.length)/text.length)/(up + mid + down));
                        console.log("MAX: " + max + " " + (up + mid + down));
                        num_up = max;
                        num_mid = max;
                        num_down = max;
                }
